...
该规划入口不重置执行预算，保守拒绝删除/重写旧验收、改变授权上限、
移除原任务/里程碑或解除没有明确答案的必要问题；P11 的活动会话重规划见文末专节。
...
请在规划阶段合入完整草稿并审阅；已有计划仍拒绝弱化验收；P11 运行中重规划也保留原验收，
...
P10 保持 IN_PROGRESS；P11 已开始实现当前批次重规划，P12 尚未开始，requirement_complete 仍为 false。
...
创建新计划可设置 --max-replans（默认 2，0 表示禁止重规划）。
运行时显式添加 --replan，使用 --review-env-file 或 --review-model 配置的 API 调用 Planner；
例如在既有 agent run 命令中加入 --replan --review-env-file .env，先预览并核对新指纹，
再按原 --approve 流程执行。启用重规划或改变 API 配置会改变执行指纹。

- Coder 仍使用原 Codex 配置；Planner 只得到需求、知识、当前上下文/Diff、
  工作流失败和累计消耗，返回 PlanDraft，无工具权限。
- 明确的检查失败/有效审查问题可触发有限修复。环境不可用、缺失/无效/不确定证据
  不触发无依据的代码修复；匹配已有基线失败时阻塞。诊断表示观察到的事实，
  code_failure 不代表已证明是本次变更引入的回归。
- 所有已开始的 Coder 尝试、审查修复、工具请求和模型调用累计到同一 Session；
  更换任务 ID 不能获得新的 Session 预算，保留 ID 的任务继续累计其尝试次数。
  --max-replans 计算已开始的重规划请求，失败也计数。审查修复上限在 Session 内共享。
  每次适配器/工作流调用的时间上限继续生效；不新增 token、金额或整次运行时长承诺。
- 仅自动继续原授权内的当前批次调整：原需求/验收/不变量/风险/权限/规则不得弱化，
  保留所有后续里程碑。扩展范围、待决事项或无效提案会留痕并停止。
  当前批次可以拆分任务、更新复杂度；后续里程碑自动激活尚未实现。
- 重规划版本保存在 .agent/run-<plan-id>/ 的不可变工件中，由 replan_proposed /
  plan_registered 事件引用；源项目 plan.json 仍指向开始执行时的计划。
  agent history 可查完整版本链，agent evidence 区分当前版本与历史证据；
  新版本重跑当前任务和适用检查，不继承旧 VERIFIED 结论。
- 预算耗尽保留工作区与全部失败记录；已有执行目录仍拒绝重跑。
  崩溃/未知副作用/用户手动修改的 reconciliation 与 resume 仍属于 P12。
  最终快照验证或审查失败仍会阻塞，requirement_complete 保持 false。

P11 已进入开发，尚未完成代表性分批重构、真实模型与跨平台验收。
Windows 适配继续按 DEV_PLAN.md 第 9.1 节 TODO 暂停。
