...
| P11 修复与重规划 | P10 | 有界修复循环、复杂度重评估、计划变更和证据失效处理 | IN_PROGRESS |
...
用户于 2026-09-17 授权开始；Windows TODO 继续暂停，P10 真实质量/跨平台待验收不计为通过。

- [x] 按验证/审查结果执行有限次数修复，记录失败原因和每次尝试。
- [x] 区分明确检查失败、已有基线失败、环境问题和无法判断的故障；不自动修复不确定结果。
- [x] 运行中重规划保留原需求、历史任务、变更原因和适用授权；代码/计划变化使旧证据失效。
- [x] 同一活动 Session 的调用、尝试、审查修复和重规划上限累计，不能通过换任务/版本重置。
- [ ] 当前批次可重评估复杂度并拆分/替换任务，记录新旧分类、消耗和未完成范围；
      后续里程碑的证据驱动激活、涉及新授权的继续执行及代表性分批重构验收仍待补齐。

**当前边界：** 显式 agent run --replan 才调用 Planner，复用配置的审查 API；
无工具、独立输入，使用同一日志和预算。只自动接纳原授权范围内的当前批次调整。
新版本保存在执行日志引用的不可变工件，不改源项目计划索引或已有会话。
未知副作用、手动修改、规则变化和扩权提案停止；不开放旧日志重写，P12 Resume 未实现。
最终快照门禁失败仍阻塞，不能借重规划省略最终检查。真实模型和跨平台 P11 验收未运行。

**验收：** 修复后重新验证/审查；预算耗尽保留可检查的工作区、日志与未完成结果；
不得通过删验收条件、改规则或无限换计划获得通过。P12 再提供恢复入口。
...
### 8.19 P11 运行中修复与重规划（2026-09-17）

用户明确授权开始 P11；基线 feb46fe，Windows 适配暂停状态不变。
复用 WorkflowEngine、PlanDraft、ModelRuntime、JsonlJournal 和既有验证/审查运行器，无新依赖。

新增结构化 FailureDiagnosis、SessionUsage 与 ReplanRecord。
明确失败可有界修复；已有基线失败、环境/不确定结果阻塞，记录失败来源与判断依据。
code_failure 表示当前检查明确失败，不声称已证明它由本次改动引入。
已开始的 Coder 尝试与审查修复从持久事件累计；换任务 ID 仍受同一 Session 总额度限制。

计划新增 --max-replans（默认 2，允许 0）；运行 --replan 显式启用配置的审查 API 作为 Planner。
重规划调用与读上下文/Diff 共用原模型/工具额度，失败提案也消耗重规划次数。
提案保留原验收、不变量、风险下限、规则和所有里程碑；当前任务可拆分/替换，
扩权或弱化检查时保留提案并停止。新旧复杂度、退役任务、消耗及旧证据处理均留痕。
接纳版本关联前版指纹并派生原授权；同一工作区继续，不重置、不丢弃修改。
Evidence 查询按各自注册版本核对历史记录，旧版本不能成为新计划的当前证据。

首次新增拆分测试因测试辅助函数提前将 staged 改成 task_graph 而失败；
已修正用例构造，未放宽产品校验。完整回归 941 passed、26 skipped、1 xfailed
（473.38 秒）；该轮启动后补强了局部验收保留与跨版本审查修复计数，
最新工作流/P11 定向检查 102 passed（55.66 秒），覆盖新增两项回归。
Ruff check/format、Windows/Darwin mypy（67 个源码文件）、CLI help、
git diff --check 通过；跳过及原 Windows 严格 xfail 不计为通过。
未调用真实模型，未重新开展 Windows 适配或安装组件。
P11 保持 IN_PROGRESS：后续里程碑激活、涉及新增授权的继续路径、
真实模型质量与跨平台验收仍待完成；P12 不启动，不提交或推送。

## 9. 下一步执行单元

当前安排（2026-09-17）：按用户要求暂停 Windows 适配，列为下方 TODO。
此前开发记录中的 Windows 优先推进顺序已被本安排替代；后续“继续”不自动恢复该项。
当前主线为 P11：下一执行单元是后续里程碑的证据驱动激活和新增授权后的继续流程，
仍需保留已有预算与失败历史。下列早期阶段复验保留为验收待办，不把未验证项计为完成。
